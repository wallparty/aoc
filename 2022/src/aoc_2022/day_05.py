...
class CrateStacks:
    ...
    def add_level(self, level: Iterator[str]) -> None:
        if self._stacks == []:
            ...
            self._stacks = list(map(call(), repeat(list, level_len)))
    ...
    def move_crates(self, amount: int, from_stack: int, to_stack: int) -> None:
        crates = list(map(call(self._stacks[from_stack - 1]), repeat(list.pop, amount)))
...
def parse_repeated_moves(line: str) -> Iterator[tuple[int, int]]:
    ...
    repeats, from_stack, to_stack = tuple(
        map(int, map(call(match), map(itemgetter, range(1, 4))))
    )
